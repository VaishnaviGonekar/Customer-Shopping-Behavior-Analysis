...
--Among all purchases of each product, what percentage were bought using a discount?
...
with item_counts as(
	select category, item_purchased, count(*) as total_purchase,
		   row_number() over (partition by category order by count(*) desc) as item_rank
	from customer
	group by category, item_purchased
)

select item_rank, category, item_purchased, total_purchase
from item_counts
where item_rank <=3;
